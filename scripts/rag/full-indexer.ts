/**
 * Full RAG indexer - Index ALL 26 workshop pages
 */
...
async function indexAllWorkshopPages(options: IndexingOptions) {
  ...
  if (verbose) {
    ...
    console.log(`📄 Pages to index: ${ALL_WORKSHOP_PAGES.length}\n`);
  }
  ...
  try {
    ...
    const pages = extractWorkshopPages(appDir, ALL_WORKSHOP_PAGES);
    ...
    const embeddings = await generateEmbeddings(texts, {
      batchSize: 10,
      delayMs: 150, // Slightly slower for large batches
...
export { indexAllWorkshopPages, ALL_WORKSHOP_PAGES };
